...
impl HandScore {
    fn new(hand: &str) -> Self {
        ...
        println!("Hand: {:?}", card_map);
        let val = if *joker_count < 5 && *joker_count > 0 {
            let mut value_without_joker = card_map
                .iter()
                .filter_map(|(key, value)| (*key != 'J').then_some(*value))
                .collect::<Vec<_>>();
            value_without_joker.sort();
            if let Some(last) = value_without_joker.last_mut() {
                *last = *last + joker_count;
            }

            value_without_joker
        } else {
            let mut values = card_map.values().map(|v| *v).collect::<Vec<_>>();
            values.sort();
            values
        };
        ...
        let hand_type = match val.as_slice() {
...
KTJJT 220
QQQJA 483";
        assert_eq!(5905, process(contents).unwrap());
    }
}
